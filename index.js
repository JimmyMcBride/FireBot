...
Head over to our **🍥role-selector** channel to help represent your style as a developer!`)
    ...
    const embed = new RichEmbed()
      .setTitle('Choose your favorite tools to code:')
      .setDescription(`
        🕶 ${html.toString()}
        🎨 ${frontEnd.toString()}
        💻 ${backEnd.toString()}
        ...
        📓 ${sql.toString()}
        🐉 ${mongo.toString()}
        🎢 ${rubyOnRails.toString()}
        🔥 ${firebase.toString()}
        🖖 ${react.toString()}
        📱 ${reactNative.toString()}
        🛡 ${angular.toString()}
      `)

    const roleChannel = member.guild.channels.find(channel => channel.name === '🍥role-selector')

    if (!roleChannel) return

    roleChannel.send(embed).then(async msg => {
